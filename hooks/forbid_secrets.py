...
def contains_secret(filename):
    with open(filename, mode="r") as file_checked:
        ...
        if kubernetes_secret:
            ignore_secret = re.findall(SOPS_REGEX, lines, flags=re.IGNORECASE | re.MULTILINE) or re.findall(KUSTOMIZE_REGEX, lines, flags=re.IGNORECASE | re.MULTILINE) or re.findall(AVP_REGEX, lines, flags=re.IGNORECASE | re.MULTILINE)
